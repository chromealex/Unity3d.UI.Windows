...
namespace UnityEngine.UI.Windows.Plugins.Heatmap.Core {
	...
	public class HeatmapSettings : ScriptableObject {
		...
		public class WindowsData {
			...
			public class Window {
				...
				public void AddPoint(Vector2 point, Vector2Int screenSize, int screenId, LayoutTag tag, WindowComponent component) {
					...
					this.points.Add(new Point(point, screenSize, screenId, tag, component));
					this.changed = true;
		...
		public string heatmapAuthKey;

		public List<AnalyticsItem> keys = new List<AnalyticsItem>();
		...
		public void AddService(IAnalyticsService service) {
			...
			item.platformName = service.GetPlatformName();
			this.keys.Add(item);

		}

		public bool HasService(string platformName) {

			for (int i = 0; i < this.keys.Count; ++i) {

				if (this.keys[i].platformName == platformName) return true;
