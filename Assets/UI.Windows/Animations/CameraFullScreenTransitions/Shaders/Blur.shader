	...
	SubShader {
		...
		Pass {
			...
			fixed4 frag( v2f_img i ) : COLOR {
				...
				sum += tex2D( _MainTex, half2( i.uv.x, i.uv.y - 4.0 * value ) ) * 0.05;
				sum += tex2D( _MainTex, half2( i.uv.x, i.uv.y - 3.0 * value ) ) * 0.09;
				sum += tex2D( _MainTex, half2( i.uv.x, i.uv.y - 2.0 * value ) ) * 0.12;
				sum += tex2D( _MainTex, half2( i.uv.x, i.uv.y - value ) ) * 0.15;
