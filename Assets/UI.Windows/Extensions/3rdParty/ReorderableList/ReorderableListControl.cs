...
namespace UnityEditor.UI.Windows.Internal.ReorderableList {
	...
	public class ReorderableListControl {
        ...
        /// import UnityEditor.UI.Windows.Internal.ReorderableList;
		...
		private void PrepareState(int controlID, IReorderableListAdaptor adaptor) {
			...
			_visibleRect = ReorderableGUIHelper.VisibleRect();
		...
		private bool DoRemoveButton(Rect position, bool visible) {
			...
			return ReorderableGUIHelper.IconButton(position, visible, iconNormal, iconActive, ItemButtonStyle);
		}
			...
			try {
				...
				if (isRepainting && isVisible) {
					...
					if (isDraggable) {
						...
						ReorderableGUIHelper.DrawTexture(texturePosition, ReorderableListResources.GetTexture(ReorderableListTexture.GrabHandle));
					}
					...
					if (!_tracking || itemIndex != s_AnchorIndex) {
						if (itemIndex != 0 || HorizontalLineAtStart) {
							...
							ReorderableGUIHelper.Separator(horizontalLinePosition, HorizontalLineColor);
						}
					}
				}
		...
		private void DrawFloatingListItem(IReorderableListAdaptor adaptor, float targetSlotPosition) {
			if (Event.current.type == EventType.Repaint) {
				...
				ReorderableGUIHelper.Separator(targetPosition, HorizontalLineColor);
				...
				ReorderableGUIHelper.DrawTexture(targetPosition, EditorGUIUtility.whiteTexture);
				...
				ReorderableGUIHelper.DrawTexture(s_DragItemPosition, EditorGUIUtility.whiteTexture);
...
//*/
		...
		private void AutoScrollTowardsMouse() {
			...
			Rect visiblePosition = ReorderableGUIHelper.VisibleRect();
			Vector2 mousePosition = Event.current.mousePosition;
		...
		protected virtual void DrawDropIndicator(Rect position) {
			ReorderableGUIHelper.Separator(position);
		}
		...
		private void CheckForAutoFocusControl() {
			...
			if (s_AutoFocusControlID == _controlID) {
				...
				ReorderableGUIHelper.FocusTextInControl("AutoFocus_" + _controlID + "_" + s_AutoFocusIndex);
				s_AutoFocusIndex = -1;
			}
		}
		...
		private void DrawFooterControls(Rect position, IReorderableListAdaptor adaptor) {
			if (HasFooterButtons) {
				...
				if (HasAddButton) {
					...
					if (ReorderableGUIHelper.IconButton(buttonPosition, true, iconNormal, iconActive, FooterButtonStyle)) {
						// Append item to list.
						GUIUtility.keyboardControl = 0;
						AddItem(adaptor);
					}
				...
				if (HasAddMenuButton) {
					// Draw add menu drop-down button.
					if (ReorderableGUIHelper.IconButton(menuButtonPosition, true, menuIconNormal, menuIconActive, FooterButtonStyle)) {
						GUIUtility.keyboardControl = 0;
